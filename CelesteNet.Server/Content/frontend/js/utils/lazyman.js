...
class LazyMan {
   ...
   * @param {string} [prefixType] The prefix type, f.e. "js", if it doesn't match the URL ending.
   * @returns {Promise<any>} A promise returning the dependency if applicable.
   */
  load(id, url, type, prefixType) {
    if (!url)
      url = id;
    ...
    if (!prefixType) {
      prefixType = type;
      let indexOfStar = url.indexOf("*");
      if (indexOfStar !== -1) {
        prefixType = url.slice(0, indexOfStar);
        url = url.slice(indexOfStar + 1);        
      }
    }

    let indexOfProtocol = url.indexOf("://");
    if (indexOfProtocol === -1 || indexOfProtocol >= 8) {
      if (this.prefixes[prefixType])
        url = this.prefixes[prefixType] + url;
   ...
   * @param {(id: string) => void} [onfulfilled] The optional per-dependency resolve callback.
   * @param {(id: string) => void} [onrejected] The optional per-dependency reject callback.
